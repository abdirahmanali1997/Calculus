...
public class EulersMethodGraph extends Graph{

	public EulersMethodGraph( CalculusApplet applet, String f, double a, double b, double c ){
		super();
	...
	public void draw( Graphics2D g ){
		...
		g.setStroke( curve );
		String s = applet.choice.getSelectedItem().toString();
		...
		F.addVariable( "y", y0 );
		m = F.getValue();
		double left = originX+w/2/scale;
		if ( s.equals("Modified Euler's Method") ){
			while ( x0 < left && i<1000 && !Double.isNaN( m ) ){
				d = c/Math.sqrt(1+m*m);
				P = toScreenPoint( x0 = x0 + d, y0 = y0 + d*m );
				path.lineTo( (float)P[0], (float)P[1] );
				if ( scale*c > 2 ) g.draw( new Ellipse2D.Double( P[0] - 1, P[1] - 1, 2, 2 ) );
				F.addVariable( "x", x0 );
				F.addVariable( "y", y0 );
				m = F.getValue();
				i++;
			...
			while ( x0 < left && i<1000 && !Double.isNaN( m ) ){
				P = toScreenPoint( x0 = x0 + c, y0 = y0 + c*m );
				path.lineTo( (float)P[0], (float)P[1] );
				if ( scale*c > 2 ) g.draw( new Ellipse2D.Double( P[0] - 1, P[1] - 1, 2, 2 ) );
				F.addVariable( "x", x0 );
				F.addVariable( "y", y0 );
		...
		F.addVariable( "x", x0 );
			F.addVariable( "y", y0 );
		m = F.getValue();
		double right = originX-w/2/scale;
		if ( s.equals("Modified Euler's Method") ){
			while ( x0 > right && i < 1000 && !Double.isNaN( m ) ){
				d = c/Math.sqrt(1+m*m);
				P = toScreenPoint( x0 = x0 - d, y0 = y0 - d*m );
				path.lineTo( (float)P[0], (float)P[1] );
				if ( scale*c > 2 ) g.draw( new Ellipse2D.Double( P[0] - 1, P[1] - 1, 2, 2 ) );
